...
# Draw atoms
ax.scatter(all_atoms_A[:, 0], all_atoms_A[:, 1], all_atoms_A[:, 2],
           color='red', s=50, label='Atom A', depthshade=False)
ax.scatter(all_atoms_B[:, 0], all_atoms_B[:, 1], all_atoms_B[:, 2],
           color='blue', s=50, label='Atom B', depthshade=False)

# Function to find the nearest atom
...
# Ensure atoms exist before selection
if len(all_atoms_A) > 0 and len(all_atoms_B) > 0:
    ref_atom_A = find_nearest_atom(all_atoms_A, [0, 0])
    ref_atom_B = find_nearest_atom(all_atoms_B, [a, 0])

    # Check if an upper-layer reference atom exists
    ...
    if len(upper_candidates) > 0:
        ...
        print("Warning: No exact reference atom found in the upper layer. Attempting closest match.")
        ref_atom_upper = find_nearest_atom(all_atoms_A, [a, 0, c])
else:
    raise ValueError("No atoms found in the dataset!")
...
ax.text((ref_atom_A[0] + ref_atom_B[0]) / 2,
        ...
        0,
        f'a = {a} Å', color='black', fontsize=12)

ax.plot([ref_atom_B[0], ref_atom_B[0]],
        [ref_atom_B[1], ref_atom_B[1]],
        [ref_atom_B[2], ref_atom_upper[2]],
        color='green', linestyle='--')
ax.text(ref_atom_B[0], ref_atom_B[1], c / 2,
        f'c = {c} Å', color='green', fontsize=12)
